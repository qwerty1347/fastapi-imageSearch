...
- **의존성 주입 / 모델 싱글톤**: `app/core/dependencies/`에서 `lru_cache`로 Qdrant 클라이언트·CLIP·YOLO를 프로세스당 1회 로드, FastAPI는 `Depends`로, Celery 워커는 동일 캐시 함수를 직접 호출하여 재사용
- **lifespan 워밍업**: 앱 부팅 시 모델·클라이언트를 미리 로드해 첫 요청 지연 제거 (`app/main.py`의 `lifespan`)
- **이벤트 루프 보호**: YOLO/CLIP/Qdrant 같은 동기 호출을 `asyncio.to_thread()`로 워커 스레드에 위임
...
상세 분석은 [`docs/REVIEW.md`](docs/REVIEW.md) 참고.

- **의존성 주입 도입**: `app/core/dependencies/common.py`에 `get_qdrant_client` / `get_embedding_model` / `get_yolo_model`을 `@lru_cache(maxsize=1)`로 정의. `dependencies/image_search.py`가 `FruitPointService` / `FruitSearchService` 팩토리를 제공하고, 라우터는 `Depends(get_fruit_search_service)`로 주입받음.
- **모델 싱글톤화**: 라우터 import 시점에 모델이 로드되던 문제 해소. 프로세스당 1회만 로드되고, FastAPI 워커와 Celery 워커 모두 동일 캐시 함수를 재사용.
- **lifespan 워밍업**: `app/main.py`에 `asynccontextmanager` 기반 lifespan 추가. 부팅 시 Qdrant 클라이언트·CLIP·YOLO를 미리 로드, 종료 시 Qdrant 클라이언트 close.
- **이벤트 루프 블로킹 해소**: `FruitSearchService.get_similarity_images`의 YOLO 추론, `create_point_data`, CLIP `encode`, Qdrant `find_points` 호출을 `asyncio.to_thread()`로 감쌌음.
- **Qdrant 래퍼 주입형 변경**: `Qdrant(client: QdrantClient)`로 시그니처 변경, 내부에서 `config`를 직접 참조하지 않음.

## ⚠️ 남은 제한 사항

- **Qdrant 컬렉션 자동 생성 없음**: `fruits` 컬렉션은 사전에 `vectors_config={"size": 768, "distance": "Cosine"}`로 생성되어 있어야 함. lifespan에서 `create_collection` 호출하도록 보강 권장.
...
- **Celery 워커 튜닝 부족**: `task_acks_late`, `worker_prefetch_multiplier=1`, `task_time_limit` 미설정. GPU 작업 안전성 위해 보강 필요.
- **테스트 부재**: `tests/` 디렉터리는 존재하지만 테스트 파일이 없음. DI 도입으로 `app.dependency_overrides`를 통한 mock 주입이 가능해진 상태.
- **Qdrant 래퍼의 추상화 가치 낮음**: 현재 `upsert_points` / `find_points`는 `qdrant_client` API의 얇은 패스스루. 도메인 리포지토리(`FruitVectorRepo`)로 승격하거나 제거 검토 필요.

### 권장 다음 단계

1. lifespan에 `client.collection_exists("fruits")` + `create_collection(...)` 추가
2. `tests/`에 라우터·서비스 단위 테스트 작성 (`app.dependency_overrides[get_qdrant_client]`로 mock 주입)
3. `Qdrant` 래퍼를 도메인 리포지토리로 승격하거나 제거
